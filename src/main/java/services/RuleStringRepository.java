...
public class RuleStringRepository {
	...
	private void Init() {
		...
		String unanimityStealsPointsRule = imports
			...
			+ " then "
				+ " logger.info(\"Everyone else has voted against \" + $proposer.getName() + \"'s rule change, so \" + $agent.getName() + \" steals 7 points from \" + $proposer.getName() + \".\"); "
